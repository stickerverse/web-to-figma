...
app.post('/scrape', async (req, res) => {
  ...
  try {
    ...
    const progressTracker = new ProgressTracker((update: ProgressUpdate) => {
...
wss.on('connection', (ws) => {
  ...
  logger.setCallback((log: LogMessage) => {
    if (ws.readyState === WebSocket.OPEN) {
  ...
  try {
    ...
    try {
      let data;
        ...
        const nodesForStreaming: IRNode[] = data.nodes;
...
async function startServer() {
  ...
  try {
    ...
    if (error?.code === 'EPERM') {
      console.error(
        `❌ Permission denied while trying to bind to port ${preferredPort}. ` +
