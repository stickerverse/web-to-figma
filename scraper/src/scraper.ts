...
import type { IRDocument, IRFontAsset, IRImageAsset, IRSVGAsset } from "../../ir.js";
import { legacyMigrator, computeBoxModelBounds, CSSInheritanceResolver, GridUtils } from "../../ir.js";
import { compileIR } from "./ir-compiler.js";
import { parseEffectsToIR, analyzeTextShadowComplexity, analyzeFilterComplexity } from "./effects-parser.js";
...
function prioritizeFontSources<T extends { url: string }>(sources: T[]): T[] {
...
function getFontPriority(extension?: string): number {
  if (!extension) return FONT_SOURCE_PRIORITY.length + 1;
  const idx = FONT_SOURCE_PRIORITY.indexOf(extension);
  return idx === -1 ? FONT_SOURCE_PRIORITY.length : idx;
}

function shouldTreatAsSystemFont(
  family: string,
  ...
  if (
    ...
    sources.every((source) =>
      RESTRICTED_FONT_HOST_PATTERNS.some((pattern) => pattern.test(source.url))
    )
  ) {
    return true;
  }
...
export interface ExtractionOptions {
  ...
  capturePhase0Screenshots?: boolean; // NEW: Enable Phase 0.5 element screenshots (default: false)
}

// ==================== NEW PHASE 0.5 INTERFACES ====================
...
interface StackingContextNode {
  ...
  ancestorTransformChain?: string[];
  paintOrderUnknown?: boolean;
  error?: string;
}

interface StackingContextTree {
  stackingContexts: {
    root: StackingContextNode;
  };
}

interface PaintOrderResult {
  paintOrder: string[];
  errors: Array<{ elementId: string; error: string }>;
  specialCases: Array<{ elementId: string; reason: string }>;
}

// ==================== HELPER FUNCTIONS ====================

function calculateScreenshotDPR(renderEnv: RenderEnv): number {
...
async function applyRenderingStabilityLayer(page: Page): Promise<void> {
  ...
  await page.evaluate((config) => {
    ...
    function freezePage() {
      ...
      if (config.FREEZE_DISABLE_SCROLL) {
        const scrollableElements = new Map<Element, { top: number; left: number }>();
      ...
      if (config.FREEZE_DISABLE_TIMING) {
        ...
        (window as any).__originalTimingFunctions = {
        ...
        (window as any).setTimeout = (callback: any, delay?: number, ...args: any[]) => {
          ...
          return -1 as any; // Return dummy timer ID
        };

        (window as any).setInterval = () => -1 as any; // Return dummy interval ID
...
async function screenshotEverything(
  ...
  enablePageScreenshot: boolean = false // NEW: Control page screenshot (default: disabled to prevent hanging)
    ...
    try {
      ...
      pageScreenshot = await Promise.race([
        ...
        new Promise<Buffer>((_, reject) =>
          setTimeout(() => reject(new Error('Page screenshot timeout')), PAGE_SCREENSHOT_TIMEOUT)
        )
      ]);
  ...
  const result: PrimaryScreenshots = {
    page: {
  ...
  try {
    totalElements = await Promise.race([
      ...
      new Promise<number>((_, reject) =>
        setTimeout(() => reject(new Error('Element count timeout')), 3000)
      )
    ]);
    console.log(`[Phase 0.5] Estimated ${totalElements} elements in DOM`);
  } catch (error) {
    console.warn(`[Phase 0.5] ⚠️  Could not estimate element count, skipping element screenshots: ${(error as Error).message}`);
    return result;
  }
  ...
  try {
    allElements = await Promise.race([
      ...
      new Promise<any[]>((_, reject) =>
        setTimeout(() => reject(new Error('Element query timeout')), ELEMENT_QUERY_TIMEOUT)
      )
    ]);
  ...
  const elementScreenshots = new Map<string, PrimaryScreenshot>();
  let capturedCount = 0;
  let skippedCount = 0;
  ...
  for (const element of elements) {
    ...
    try {
      ...
      const captureWithTimeout = async () => {
        ...
        const elementId = await element.evaluate((el: Element) => {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Element screenshot timeout')), ELEMENT_TIMEOUT)
        )
      ]);
  ...
  console.log(
    `[Phase 0.5] ✅ Captured ${capturedCount} screenshots (skipped ${skippedCount}) in ${(duration / 1000).toFixed(1)}s`
  );

  // Store in page context
  await page.evaluate((screenshots) => {
    (window as any).__primaryScreenshots = screenshots;
...
async function waitForFullyLoaded(page: Page): Promise<LoadInfo> {
  ...
  const timestamps: LoadInfo["timestamps"] = {
    documentReady: 0,
    fontsReady: 0,
    imagesReady: 0,
    lazyContentReady: 0,
  ...
  try {
    ...
    try {
      const imageLoadPromise = page.evaluate(async () => {
        ...
        const incompleteImages = Array.from(allImages).filter(
    ...
    try {
      const lazyLoadPromise = page.evaluate(async () => {
        ...
        const newIncompleteImages = Array.from(newImages).filter(
          (img) => !img.complete || img.naturalWidth === 0
        );
    ...
    try {
      const stabilizationResult = await page.evaluate(async () => {
        ...
        while (checks < 3 && !stable) {
          ...
          const changePercent =
            (Math.abs(newLength - lastLength) / lastLength) * 100;
...
async function captureRenderEnvironment(page: Page): Promise<RenderEnv> {
  const renderEnv = await page.evaluate(() => {
    ...
    const innerHeight = safeGet(
      () => window.innerHeight,
      0,
      "window.innerHeight"
    );
    const outerWidth = safeGet(() => window.outerWidth, 0, "window.outerWidth");
    const outerHeight = safeGet(
      () => window.outerHeight,
      0,
      "window.outerHeight"
    );
    const devicePixelRatio = safeGet(
      () => window.devicePixelRatio,
      1,
      "window.devicePixelRatio"
    );
    const screenWidth = safeGet(
      () => window.screen.width,
      0,
      "window.screen.width"
    );
    const screenHeight = safeGet(
      () => window.screen.height,
      0,
      "window.screen.height"
    );

    const docClientWidth = safeGet(
      () => document.documentElement.clientWidth,
      0,
      "document.documentElement.clientWidth"
    );
    const docClientHeight = safeGet(
      () => document.documentElement.clientHeight,
      0,
      "document.documentElement.clientHeight"
    );
    const docScrollWidth = safeGet(
      () => document.documentElement.scrollWidth,
      0,
      "document.documentElement.scrollWidth"
    );
    const docScrollHeight = safeGet(
      () => document.documentElement.scrollHeight,
      0,
      "document.documentElement.scrollHeight"
    );

    const bodyClientWidth = safeGet(
      () => document.body?.clientWidth,
      0,
      "document.body.clientWidth"
    );
    const bodyClientHeight = safeGet(
    ...
    const language = safeGet(
      () => navigator.language,
      "",
      "navigator.language"
    );
    const hardwareConcurrency = safeGet(
      () => navigator.hardwareConcurrency,
      0,
      "navigator.hardwareConcurrency"
    );
    const maxTouchPoints = safeGet(
    ...
    try {
      if (window.matchMedia) {
        if (
          ...
          safeGet(() => window.matchMedia("(color-gamut: p3)").matches, false)
        ) {
          colorGamut = "p3";
        } else if (
          safeGet(() => window.matchMedia("(color-gamut: srgb)").matches, false)
        ) {
          colorGamut = "srgb";
        }
...
function shouldRasterizeText(node: any): string | null {
  ...
  const fontSize = parseFloat(
    typography.font?.size || typography.fontSize || "0"
  );
  if (
    ["h1", "h2"].includes(node.tag || "") &&
  ...
  // Non-system fonts
  const fontFamily = (
    typography.font?.familyResolved ||
  ...
  const letterSpacing =
    typography.layout?.letterSpacing || typography.letterSpacing || "0";
  ...
  if (
    typography.effects?.gradientText ||
...
async function rasterizeElementText(
  page: Page,
  node: any,
  reason: string,
  renderEnv: RenderEnv
...
async function processTextRasterization(
  page: Page,
  nodes: any[],
  renderEnv: RenderEnv
): Promise<{ nodes: any[]; stats: RasterStats }> {
  ...
  const candidates: Array<{ node: any; reason: string; index: number }> = [];
  ...
  console.log(
    `[Phase 6] Found ${candidates.length} elements qualifying for rasterization`
  );
    ...
    toRasterize = candidates
      .sort((a, b) => {
        ...
        const aSize = parseFloat(
          a.node.typography?.font?.size || a.node.typography?.fontSize || "0"
        );
        const bSize = parseFloat(
          b.node.typography?.font?.size || b.node.typography?.fontSize || "0"
        );
        if (Math.abs(aSize - bSize) > 2) return bSize - aSize;
  ...
  for (const candidate of toRasterize) {
    try {
      const result = await rasterizeElementText(
        page,
        candidate.node,
        candidate.reason,
        renderEnv
  ...
  if (stats.rasterized > 0) {
    stats.averageImageSize = Math.round(
      stats.totalImageSize / stats.rasterized
    );
  }
  ...
  console.log(
    `[Phase 6] ✅ Completed: ${
      stats.rasterized
    } rasterized in ${duration.toFixed(0)}ms`
  );
...
function cssShadowToFigmaEffects(boxShadow: string): FigmaEffect[] {
...
function cssToFigmaAutoLayout(
  styles: Record<string, any>
): FigmaAutoLayout | null {
...
function convertToFigmaProperties(node: any): FigmaProperties {
  const styles = node.styles || {};
  const figmaProps: FigmaProperties = {
  ...
  if (fills.length > 0) figmaProps.fills = fills;
  ...
  if (effects.length > 0) figmaProps.effects = effects;
  ...
  if (autoLayout) figmaProps.autoLayout = autoLayout;
  ...
  if (node.type === "TEXT" && node.typography) {
    ...
    const mainFamily =
      families.find(
        (f: string) =>
          !["serif", "sans-serif", "monospace"].includes(f.toLowerCase())
      ) || families[0];

    const textContent =
      typo.text?.content || typo.content?.text || node.text || "";
    const fontSize = parseFloat(typo.font?.size || typo.fontSize || "16");
    const lineHeightPx =
      typo.layout?.lineHeightPx || typo.lineHeightPx || fontSize * 1.2;
    const letterSpacing = parseFloat(
      typo.layout?.letterSpacing || typo.letterSpacing || "0"
    );
    const textAlign = typo.layout?.align || typo.textAlign || "left";
    const color = cssColorToFigma(
      typo.effects?.color || typo.color || "#000000"
    );
    ...
    figmaProps.textData = {
      ...
      fontName: {
        family: mainFamily,
        style:
          (typo.font?.style || typo.fontStyle || "normal") === "italic"
            ? "Italic"
            : "Regular",
      },
      fontSize,
      ...
      textAlignHorizontal: textAlign.toUpperCase() as any,
...
function calculateConfidenceScore(node: any): number {
  let confidence = 1.0;

  if (node.typography) {
    if (
      node.typography.effects?.gradientText ||
      node.typography.specialCases?.isGradientText
    ) {
      confidence -= 0.15;
    }
    if (
      node.typography.effects?.stroke ||
      node.typography.effects?.strokeWidth
    ) {
      confidence -= 0.1;
    }
    const shadow =
      node.typography.effects?.shadow || node.typography.textShadow;
    if (shadow && shadow !== "none") {
      confidence -= 0.05;
    }

    const fontFamily = (
      node.typography.font?.familyResolved ||
...
function validateReconstruction(node: any): ValidationResult {
  ...
  if (useFallback) {
    ...
    const mixBlendMode =
      node.compositing?.mixBlendMode || node.styles?.mixBlendMode;
    if (mixBlendMode && mixBlendMode !== "normal") {
...
function analyzeOptimization(node: any, allNodes: any[]): OptimizationHints {
...
function isStackingContext(
  ...
  try {
    ...
    if (willChange && willChange !== "auto") {
      ...
      if (
        willChangeValues.includes("transform") ||
    ...
    const webkitOverflowScrolling = (computedStyle as any)["-webkit-overflow-scrolling"];
...
function captureCompositingProperties(
  element: Element,
  computedStyle: CSSStyleDeclaration
): any {
  try {
    ...
    let perspectiveOrigin: string | undefined;
    if (perspective !== "none") {
      try {
...
function captureLayoutGeometry(
  element: Element,
  renderEnv: any
): LayoutGeometry {
  ...
  try {
    ...
    const compositingProperties = captureCompositingProperties(
      element,
      computedStyle
    );

    const layout: LayoutGeometry = {
      viewport: {
        x: roundToTwo(bbox.x),
        y: roundToTwo(bbox.y),
...
/**
 * Comprehensive Flexbox Layout Resolver
 * Analyzes and resolves flexbox layout according to W3C CSS Flexible Box Layout Module Level 1
 * Works in browser context to access real DOM and computed styles for accurate layout measurement
 * 
 * @param el - HTMLElement to analyze
 * @param styles - Computed styles for the element
 * @returns Flexbox layout data compatible with IRLayout interface
 */
type IRChildAlignment = import('../../ir.js').IRLayout['childAlignment'];

function normalizeMainAxisAlignment(value: string): NonNullable<IRChildAlignment>['mainAxis'] {
  ...
  if (
    ...
    return normalized as NonNullable<IRChildAlignment>['mainAxis'];
  }
  
  return 'start';
}

function normalizeCrossAxisAlignment(value: string): NonNullable<IRChildAlignment>['crossAxis'] {
  const normalized = (value || '').toLowerCase().trim();
...
function computeFlexChildAlignment(
  parentStyles: CSSStyleDeclaration,
  childStyles: CSSStyleDeclaration
): IRChildAlignment {
  const direction = parentStyles.flexDirection || 'row';
  const wrapMode = parentStyles.flexWrap || 'nowrap';
  const mainAxisOrientation: NonNullable<IRChildAlignment>['mainAxisOrientation'] =
    direction.startsWith('column') ? 'vertical' : 'horizontal';
  const crossAxisOrientation: NonNullable<IRChildAlignment>['crossAxisOrientation'] =
    mainAxisOrientation === 'horizontal' ? 'vertical' : 'horizontal';
...
function computeGridChildAlignment(
  parentStyles: CSSStyleDeclaration,
  childStyles: CSSStyleDeclaration
): IRChildAlignment {
...
function resolveFlexLayout(el: HTMLElement, styles: CSSStyleDeclaration): {
  isFlexContainer: boolean;
  isFlexItem: boolean;
  containerData?: NonNullable<import('../../ir.js').IRLayout['flex']>;
  itemData?: NonNullable<import('../../ir.js').IRLayout['flexItem']>;
  childAlignment?: IRChildAlignment;
} {
  ...
  let containerData: NonNullable<import('../../ir.js').IRLayout['flex']> | undefined;
  let itemData: NonNullable<import('../../ir.js').IRLayout['flexItem']> | undefined;
  let childAlignment: IRChildAlignment | undefined;
  ...
  if (isFlexContainer) {
    try {
      ...
      const direction = ['row', 'row-reverse', 'column', 'column-reverse'].includes(flexDirection) 
        ? flexDirection as NonNullable<import('../../ir.js').IRLayout['flex']>['direction']
        : 'row';

      // Parse flex-wrap with fallback
      const flexWrap = styles.flexWrap || 'nowrap';
      const wrap = ['nowrap', 'wrap', 'wrap-reverse'].includes(flexWrap)
        ? flexWrap as NonNullable<import('../../ir.js').IRLayout['flex']>['wrap']
        : 'nowrap';
      ...
      const justifyContent = validJustifyValues.includes(justifyContentValue)
        ? justifyContentValue as NonNullable<import('../../ir.js').IRLayout['flex']>['justifyContent']
        : 'flex-start';
      ...
      const alignItems = validAlignItemsValues.includes(alignItemsValue)
        ? alignItemsValue as NonNullable<import('../../ir.js').IRLayout['flex']>['alignItems']
        : 'stretch';
      ...
      const alignContent = validAlignContentValues.includes(alignContentValue)
        ? alignContentValue as NonNullable<import('../../ir.js').IRLayout['flex']>['alignContent']
        : 'stretch';
  ...
  if (isFlexItem && parentElement && parentStyles) {
    try {
      ...
      let grow: number, shrink: number, basis: string;
      ...
      const alignSelf = validAlignSelfValues.includes(alignSelfValue)
        ? alignSelfValue as NonNullable<import('../../ir.js').IRLayout['flexItem']>['alignSelf']
        : 'auto';
 ...
 */
function parseFlexShorthand(flex: string): { grow: number; shrink: number; basis: string } {
 ...
 */
function detectLegacyFlexGap(container: HTMLElement): { row: number; column: number } {
  if (container.children.length <= 1) return { row: 0, column: 0 };
  ...
  try {
    ...
    const marginData: { top: number; right: number; bottom: number; left: number }[] = [];
 ...
 */
function createIRLayoutFromLegacy(
  legacyLayout: LayoutGeometry,
  flexData: ReturnType<typeof resolveFlexLayout>,
  styles: CSSStyleDeclaration
): import('../../ir.js').IRLayout {
  // Parse spacing values from CSS
  const parseSpacing = (property: string): import('../../ir.js').IRSpacing => {
  ...
  const irLayout: import('../../ir.js').IRLayout = {
    // Box model
    boxModel: {
      margin: parseSpacing('margin'),
      padding: parseSpacing('padding'),
      border: parseSpacing('border-width'),
      boxSizing: (styles.boxSizing || 'content-box') as 'content-box' | 'border-box'
    },

    // Position and size  
    position: {
      type: (styles.position || 'static') as import('../../ir.js').IRLayout['position']['type'],
    ...
    display: {
      ...
      overflow: {
        x: (styles.overflowX || 'visible') as import('../../ir.js').IRLayout['display']['overflow']['x'],
        y: (styles.overflowY || 'visible') as import('../../ir.js').IRLayout['display']['overflow']['y']
      }
    },
    ...
    transform: legacyLayout.transform?.hasTransform ? {
      ...
      style: (styles.transformStyle || 'flat') as 'flat' | 'preserve-3d'
    } : undefined,
    ...
    stacking: {
      ...
      paintOrder: (legacyLayout as any).compositing?.paintOrder || 0,
 ...
 */
function parseTransformMatrix(transform: string): number[] {
  if (!transform || transform === 'none') {
 ...
 *          [0 0 1]   [0     0     1         ]
 */
function buildStackingContextTree(nodes: any[]): { 
  stackingContexts: any[], 
  paintOrder: string[] 
} {

  const nodeMap = new Map(nodes.map((n: any) => [n.id, n]));
  const stackingContexts: any[] = [];
  const stackingContextMap = new Map<string, string>(); // nodeId -> contextId
  ...
  if (!rootNode) {
    return {
      stackingContexts: [],
      paintOrder: nodes.map((n: any) => n.id),
    };
  }
  ...
  function createStackingContext(
    ...
    function collectNodesInContext(nodeId: string, currentContextId: string) {
  ...
  function computePaintOrderForContext(contextId: string) {
    const context = stackingContexts.find(ctx => ctx.id === contextId);
    if (!context) return;

    // Get all nodes in this context
    const contextNodes = context.nodeIds.map((id: string) => nodeMap.get(id)).filter(Boolean);
    
    // Separate into z-index groups
    const negativeZNodes: any[] = [];
    const autoZNodes: any[] = [];
    const positiveZNodes: any[] = [];
...
function computePaintOrder(
  stackingTree: StackingContextTree,
  nodes: any[]
): PaintOrderResult {
  const paintOrder: string[] = [];
  const errors: Array<{ elementId: string; error: string }> = [];
  const specialCases: Array<{ elementId: string; reason: string }> = [];

  const findNode = (id: string) => nodes.find((n) => n.id === id);

  function traverseStackingTree(context: StackingContextNode) {
    try {
      paintOrder.push(context.elementId);

      const negativeChildren: StackingContextNode[] = [];
      const autoChildren: StackingContextNode[] = [];
      const positiveChildren: StackingContextNode[] = [];
 ...
 */
async function extractFontsAsIRAssets(
  page: Page,
  baseUrl: string
): Promise<IRFontAsset[]> {
        ...
        try {
          ...
          for (const rule of Array.from(rules)) {
            if (rule instanceof CSSFontFaceRule) {
              ...
              const sources: { url: string; format?: string }[] = [];
  ...
  const irFontAssets = await Promise.all(
    fontFaces.map(async (fontFace, index) => {
      ...
      const normalizedSources = prioritizedSources.map((source: any) => ({
      ...
      try {
        ...
        if (skipDownload) {
          console.log(
            `[Font Extraction] 🔒 Skipping remote download for ${fontFace.family} (using system font)`
          );
        } else {
 ...
 */
function updateFontUsageInNodes(fontAssets: IRFontAsset[], extractedData: any): IRFontAsset[] {
  console.log("[Font Usage] Analyzing font usage across nodes...");
  
  const nodes = extractedData.nodes || [];
  const normalizedFontAssets: IRFontAsset[] = [...fontAssets];
  const fontUsageMap = new Map<string, Set<string>>(); // fontId -> Set of nodeIds

  const buildFontKey = (family: string, weight: string, style: string) =>
    `${family.toLowerCase()}__${weight}__${style}`;

  const fontAssetMap = new Map<string, IRFontAsset>();
  normalizedFontAssets.forEach((font) => {
    ...
    fontUsageMap.set(font.id, new Set<string>());
  });
  ...
  nodes.forEach((node: any) => {
    ...
    if (!matchingFont && isSystemFont) {
      ...
      fontUsageMap.set(matchingFont.id, new Set<string>());
    }
    ...
    if (matchingFont) {
      ...
      if (!isSystemFont) {
        console.warn(
          `[Font Usage] ⚠️ Node ${node.id} uses untracked font: ${cleanedFamily} (${fontWeight} ${fontStyle})`
        );
      }
    }
  });
 ...
 */
async function collectAllAssets(page: Page, baseUrl: string): Promise<{
  imageAssets: IRImageAsset[];
  svgAssets: IRSVGAsset[];
}> {
...
class SVGCache {
  ...
  private normalize(svgString: string): string {
    return svgString
      .replace(/\s+/g, " ")
      .replace(/id="[^"]*"/g, "")
  ...
  register(
    svgId: string,
    svgString: string
  ): { isDuplicate: boolean; originalId: string; hash: string } {
 ...
 */
async function processImageAssets(rawAssets: any[], baseUrl: string): Promise<IRImageAsset[]> {
  console.log(`[Asset Processing] Processing ${rawAssets.length} image assets...`);
  
  const processedAssets: IRImageAsset[] = [];
  const hashCache = new Map<string, IRImageAsset>(); // For deduplication
    ...
    try {
      ...
      if (imageData) {
        ...
        const {
          ...
          dimensions,
        } = await processDataURL(imageData, dimensionHints);
        buffer = processedBuffer;
        mimeType = detectedMimeType;
        ...
        const {
          ...
          mimeType: detectedMimeType,
          dimensions,
        } = await downloadAndProcessImage(resolvedUrl, dimensionHints);
        buffer = processedBuffer;
        mimeType = detectedMimeType;
        width = dimensions.width;
        height = dimensions.height;
 ...
 * Process SVG assets
 */
async function processSVGAssets(rawSvgs: any[]): Promise<IRSVGAsset[]> {
  console.log(`[SVG Processing] Processing ${rawSvgs.length} SVG assets...`);
  
  const processedSvgs: IRSVGAsset[] = [];
  ...
  for (const svgData of rawSvgs) {
    try {
      ...
      if (registration.isDuplicate) {
        console.log(
          `[SVG Processing] Duplicate SVG found: ${svgData.id} (matches ${registration.originalId})`
        );
        continue;
      }
 ...
 */
function extractSVGPaths(svgContent: string): Array<{
  d: string;
  fill?: string;
  stroke?: string;
  strokeWidth?: string;
 ...
 */
function getFormatFromMimeType(mimeType: string): "png" | "jpeg" | "webp" | "gif" | "svg" {
...
function getAssetDimensionHints(asset: any): DimensionHints {
  const width =
    normalizeDimension(asset.width) ??
    normalizeDimension(asset.naturalWidth) ??
    normalizeDimension(asset.displayWidth) ??
    normalizeDimension(asset.rect?.width);
  const height =
    normalizeDimension(asset.height) ??
...
async function detectFontFaces(page: Page, nodes: any[]): Promise<FontFace[]> {
  const fontFaceData = await page.evaluate((nodeData) => {
    ...
    const declaredFonts = new Map<string, any>();
        ...
        try {
          ...
          for (const rule of Array.from(rules)) {
            if (rule instanceof CSSFontFaceRule) {
              ...
              const fontStyle =
                style.getPropertyValue("font-style") || "normal";
              const unicodeRange =
                style.getPropertyValue("unicode-range") || undefined;
              const display =
                style.getPropertyValue("font-display") || undefined;
    ...
    const fontLoadStatus = new Map<string, string>();

    if (document.fonts) {
      try {
        document.fonts.forEach((fontFace) => {
          const normalizedFamily = normalizeFontFamily(fontFace.family);
          const key = `${normalizedFamily}-${fontFace.weight || "400"}-${
    ...
    for (const [key, fontData] of declaredFonts) {
      ...
      systemFontFaces.push({
        ...fontData,
        loadStatus: loadStatus as "loaded" | "loading" | "unloaded" | "error",
        usedByElements,
      });
    }
...
interface ScreenshotPrefilterStats {
  total: number;
  eligible: number;
  hidden: number;
  zeroSize: number;
...
function shouldScreenshotNodeCandidate(node: any): boolean {
...
function prepareScreenshotCandidates(
  nodes: any[]
): { nodes: any[]; stats: ScreenshotPrefilterStats } {
  const stats: ScreenshotPrefilterStats = {
    total: nodes.length,
    eligible: 0,
    hidden: 0,
    zeroSize: 0,
...
function measureNodeDimension(
  ...
  const viewportValue =
    axis === "width" ? viewport.width : viewport.height;
  const layoutRectValue =
    axis === "width" ? rect.width : rect.height;
  const nodeRectValue =
    axis === "width" ? nodeRect.width : nodeRect.height;
  const styleValue =
    axis === "width" ? styles.width : styles.height;

  return (
    normalizeDimension(viewportValue) ??
...
async function detectOcclusion(
  element: ElementHandle<Element>
): Promise<{ occluded: boolean; occluderIds: string[] }> {
  return element
    .evaluate((el) => {
...
async function temporarilyHideOccluders(
  page: Page,
  occluderIds: string[]
  ...
  const handles = (
    await Promise.all(
      occluderIds.map((id) => page.$(`[data-w2f-occluder="${id}"]`))
    )
  ).filter(Boolean) as ElementHandle<HTMLElement | SVGElement>[];

  if (handles.length === 0) {
    return null;
  }

  const activeHandles: ElementHandle<Element>[] = [];
  ...
  for (const handle of handles) {
    const prev = await handle.evaluate((node: HTMLElement | SVGElement) => {
  ...
  return async () => {
    await Promise.all(
      activeHandles.map((handle, index) =>
        handle.evaluate((node: HTMLElement | SVGElement, prev) => {
...
type ScreenshotSkipReason = "selectorMissing" | ScreenshotFailureReason;

interface ScreenshotCaptureSuccess {
  success: true;
  buffer: Buffer;
...
type ScreenshotCaptureResult =
  ...
  | { success: false; reason: ScreenshotFailureReason };

async function captureElementScreenshotBuffer(
  page: Page,
  element: ElementHandle<Element>,
  nodeId: string,
  opts: { dpr: number; padding?: number }
): Promise<ScreenshotCaptureResult> {
  let occlusionCleanup: (() => Promise<void>) | null = null;
  ...
  if (occlusionInfo.occluded) {
    ...
    if (!occlusionCleanup) {
      console.warn(
        `⚠️  Node ${nodeId} appears occluded but no removable occluder was found; capturing anyway`
      );
    } else {
...
async function captureElementScreenshots(
  ...
  nodes: any[],
  renderEnv: RenderEnv
): Promise<Record<string, EnhancedScreenshot>> {
  const screenshots: Record<string, EnhancedScreenshot> = {};
  const batchSize = 10;
  const screenshotDPR = calculateScreenshotDPR(renderEnv);
  ...
  let capturedCount = 0;
  let failedCount = 0;
  const skipStats: Record<ScreenshotSkipReason, number> = {
    selectorMissing: 0,
    noBoundingBox: 0,
    zeroSize: 0,
    invisibleStyle: 0,
  ...
  const skipMessage = (reason: ScreenshotSkipReason, nodeId: string): string => {
    switch (reason) {
  ...
  const logSkipWarning = (reason: ScreenshotSkipReason, nodeId: string) => {
    skipStats[reason] += 1;
    if (skipStats[reason] <= SKIP_WARN_LIMIT) {
      console.warn(skipMessage(reason, nodeId));
      if (skipStats[reason] === SKIP_WARN_LIMIT) {
        console.warn(
          `[Phase 3] ⚠️  Suppressing further '${reason}' warnings (${SKIP_WARN_LIMIT}+ hits)`
        );
      }
    }
  };
    ...
    await Promise.all(
      batch.map(async (node) => {
        try {
          ...
          const captureResult = await captureElementScreenshotBuffer(
            page,
            element,
            node.id,
            { dpr: screenshotDPR, padding: 2 }
          );
          ...
          failedCount++;
          console.warn(`⚠️  Screenshot failed for node ${node.id}: ${(e as Error).message}`);
  ...
  if (skippedTotal > 0) {
    console.log(
      `[Phase 3] ⚠️  Skipped ${skippedTotal} screenshot targets (selector: ${skipStats.selectorMissing}, bbox: ${skipStats.noBoundingBox}, zero-size: ${skipStats.zeroSize}, invisible: ${skipStats.invisibleStyle}, occluded: ${skipStats.occluded}, capture failed: ${skipStats.captureFailed})`
    );
  }
...
async function applyPageScreenshotFallback(
  nodes: any[],
  pageScreenshot: PrimaryScreenshot,
  screenshots: Record<string, EnhancedScreenshot>
): Promise<void> {
  ...
  const imageWidth =
    metadata.width ?? Math.round((pageScreenshot.width || 0) * pageDpr);
  const imageHeight =
    metadata.height ?? Math.round((pageScreenshot.height || 0) * pageDpr);
  ...
  for (const node of nodes) {
    ...
    try {
      ...
      skipped += 1;
      console.warn(
        `[Phase 3] ⚠️  Fallback screenshot failed for node ${node.id}: ${(error as Error).message}`
      );
    }
  }
  ...
  if (generated > 0) {
    console.log(
      `[Phase 3] 🔁 Generated ${generated} fallback screenshots from page capture (skipped ${skipped})`
    );
  }
}
...
async function captureInteractionStates(
  ...
  nodes: any[],
  renderEnv: RenderEnv
): Promise<Record<string, Record<string, EnhancedScreenshot>>> {
  const states: Record<string, Record<string, EnhancedScreenshot>> = {};
  const screenshotDPR = calculateScreenshotDPR(renderEnv);
  ...
  for (const node of nodes) {
    try {
      ...
      const nodeStates: Record<string, EnhancedScreenshot> = {};

      const createEnhancedScreenshot = async (
        screenshotBuffer: Buffer
      ): Promise<EnhancedScreenshot | null> => {
        const rect = await element.boundingBox();
        if (rect) {
      ...
      if (node.componentHint === "button") {
        try {
          ...
          const enhancedActive = await createEnhancedScreenshot(
            activeScreenshot
          );
          if (enhancedActive) {
...
export async function extractComplete(
  ...
  const {
    captureFonts = true,
    captureScreenshots = false, // Default: DISABLED to prevent timeout (was true)
    screenshotComplexOnly = true,
    captureStates = false,
    capturePseudoElements = true,
    extractSVG = true,
    viewport = { width: 1440, height: 900 },
    capturePhase0Screenshots = false, // Default: disabled to prevent hanging
  } = options;
  ...
  try {
    ...
    await page.addInitScript(() => {
      ...
      if (!global.__name) {
        global.__name = (target: any) => target;
      }
    });
  ...
  try {
    ...
    console.error(
      "❌ captureRenderEnvironment() failed, using fallback:",
      error
    );
    renderEnv = {
      viewport: {
  ...
  try {
    data = await Promise.race([
       ...
       */
      const captureCompositingProperties = (
        element: Element,
        computedStyle: CSSStyleDeclaration
      ): any => {
        try {
       ...
       */
      const captureLayoutGeometry = (element: Element, renderEnv: any) => {
        const roundToTwo = (num: number): number => Math.round(num * 100) / 100;
      ...
      const getTypographyValue = (
        ...
        try {
          const value =
            styles.getPropertyValue(property) || (styles as any)[property];
       ...
       */
      let typographyCapabilitiesCache: any | null = null;
      const detectTypographyCapabilities = (): any => {
        ...
        try {
          ...
          (testElement.style as any).textWrapMode = "wrap";
          ...
          (testElement.style as any).textDecorationThickness = "auto";
          capabilities.supportsTextDecorationThickness = !!(
            testElement.style as any
          ).textDecorationThickness;

          (testElement.style as any).textUnderlineOffset = "auto";
          capabilities.supportsTextUnderlineOffset = !!(
            testElement.style as any
          ).textUnderlineOffset;

          (testElement.style as any).webkitFontSmoothing = "antialiased";
          capabilities.supportsWebkitFontSmoothing = !!(
            testElement.style as any
          ).webkitFontSmoothing;
        } catch (error) {
          // Keep defaults
       ...
       */
      const calculatePreciseLineCount = (
        element: Element,
        styles: CSSStyleDeclaration
      ): number => {
        try {
          ...
          if (
            ...
            try {
              ...
              if (rangeRect.height > 0) {
                const preciseLines = Math.ceil(
                  rangeRect.height / lineHeightValue
                );
       ...
       */
      const detectGradientText = (
        styles: CSSStyleDeclaration
      ): {
        isGradient: boolean;
        gradientData?: { gradient: string; clip: boolean };
      } => {
        try {
          const backgroundImage = styles.backgroundImage;
          const webkitBackgroundClip = (styles as any).webkitBackgroundClip;
       ...
       */
      const detectTextDirections = (
        styles: CSSStyleDeclaration
      ): { isRTL: boolean; isVertical: boolean } => {
        try {
       ...
       */
      const extractTextMetrics = (el: HTMLElement): any => {
        try {
          ...
          return {
            ...
            layout: {
              ...
              direction: (computedStyle.direction || 'ltr') as "ltr" | "rtl",
       ...
       */
      const captureInputState = (element: Element): any => {
        try {
      ...
      const shouldCaptureTypography = (
        ...
        try {
          ...
          if (element.tagName) {
            ...
            if (
              typographyRelevantTags.includes(element.tagName.toUpperCase())
            ) {
              return true;
            }
          }
       ...
       */
      const getFirstLineHeight = (element: Element): number => {
        try {
       ...
       * Extract all styles
       */
      const extractAllStyles = (styles: CSSStyleDeclaration, el: Element) => {
        return {
       ...
       */
      const extractPseudoElement = (el: HTMLElement, which: "::before" | "::after"): any | undefined => {
        try {
          ...
          const hasVisualStyles = 
            pseudoStyles.backgroundColor !== "rgba(0, 0, 0, 0)" &&
          ...
          if (hasContent && content !== '""') {
            ...
            pseudoNode.textMetrics = {
              ...
              layout: {
                align: (pseudoStyles.textAlign || "left") as "left" | "right" | "center" | "justify",
                ...
                direction: (pseudoStyles.direction || "ltr") as "ltr" | "rtl",
      ...
      const needsScreenshot = (
        ...
        const hasBackgroundImage =
          backgroundImage !== "none" && backgroundImage.trim().length > 0;
       ...
       */
      function detectLegacyFlexGap(container: HTMLElement): { row: number; column: number } {
        if (!container || container.children.length <= 1) return { row: 0, column: 0 };

        try {
          let rowGap = 0;
          let columnGap = 0;
          const sampleSize = Math.min(4, container.children.length);
          const marginData: Array<{ top: number; right: number; bottom: number; left: number }> = [];
      ...
      /**
       * Minimal grid utility helpers for browser context
       */
      type GridLayout = import("../../ir.js").IRLayout["grid"];
      type GridUtilsType = {
        isGridDisplay: (display?: string) => boolean;
        normalizeJustifyContent: (value?: string) => NonNullable<GridLayout>["justifyContent"];
        normalizeAlignContent: (value?: string) => NonNullable<GridLayout>["alignContent"];
        normalizeJustifyItems: (value?: string) => NonNullable<GridLayout>["justifyItems"];
        normalizeAlignItems: (value?: string) => NonNullable<GridLayout>["alignItems"];
        normalizeJustifySelf: (value?: string) => NonNullable<GridLayout>["justifySelf"];
        normalizeAlignSelf: (value?: string) => NonNullable<GridLayout>["alignSelf"];
      };

      const GridUtils: GridUtilsType = (() => {
        ...
        const normalizeJustifyContent = (value?: string): NonNullable<GridLayout>["justifyContent"] => {
          const keyword = sanitizeKeyword(value);
          return (JUSTIFY_CONTENT_VALUES.includes(keyword as any) ? keyword : "start") as NonNullable<GridLayout>["justifyContent"];
        };

        const normalizeAlignContent = (value?: string): NonNullable<GridLayout>["alignContent"] => {
          const keyword = sanitizeKeyword(value);
          return (JUSTIFY_CONTENT_VALUES.includes(keyword as any) ? keyword : "stretch") as NonNullable<GridLayout>["alignContent"];
        };

        const normalizeJustifyItems = (value?: string): NonNullable<GridLayout>["justifyItems"] => {
          const keyword = sanitizeKeyword(value);
          return (JUSTIFY_ITEMS_VALUES.includes(keyword as any) ? keyword : "stretch") as NonNullable<GridLayout>["justifyItems"];
        };

        const normalizeAlignItems = (value?: string): NonNullable<GridLayout>["alignItems"] => {
          const keyword = sanitizeKeyword(value);
          return (ALIGN_ITEMS_VALUES.includes(keyword as any) ? keyword : "stretch") as NonNullable<GridLayout>["alignItems"];
        };

        const normalizeJustifySelf = (value?: string): NonNullable<GridLayout>["justifySelf"] => {
          const keyword = sanitizeKeyword(value);
          if (JUSTIFY_SELF_VALUES.includes(keyword as any)) return keyword as NonNullable<GridLayout>["justifySelf"];
          if (keyword === "normal") return "auto";
          return "auto";
        };

        const normalizeAlignSelf = (value?: string): NonNullable<GridLayout>["alignSelf"] => {
          const keyword = sanitizeKeyword(value);
          if (ALIGN_SELF_VALUES.includes(keyword as any)) return keyword as NonNullable<GridLayout>["alignSelf"];
          if (keyword === "normal") return "auto";
          return "auto";
        };

        const isGridDisplay = (display?: string) => display === "grid" || display === "inline-grid";
      ...
      function resolveFlexLayout(el: HTMLElement, styles: CSSStyleDeclaration) {
        ...
        let containerData: any | undefined;
        let itemData: any | undefined;
        let childAlignment: any | undefined;
          ...
          if (parentStyles.display === "flex" || parentStyles.display === "inline-flex") {
            ...
            childAlignment = {
              mainAxis: ["flex-end", "end", "self-end", "right"].includes(parentJustify) ? "end" :
                ...
                ["space-between", "space-around", "space-evenly"].includes(parentJustify) ? parentJustify as any :
                "start",
      ...
      function createIRLayoutFromLegacy(
        ...
        const parseSpacing = (property: string) => {
          const value = styles.getPropertyValue(property) || "0";
          if (!value || value === "0" || value === "auto") {
            return { top: 0, right: 0, bottom: 0, left: 0 };
          }
       ...
       */
      const computeWorldMatrix = (element: Element): number[] => {
      ...
      for (let i = 0; i < elementsToProcess; i++) {
        ...
        const textMetrics = hasTypography && extractedText
          ? extractTextMetrics(el as HTMLElement)
          : undefined;

        let nodeType: "IMAGE" | "SVG" | "CANVAS" | "VIDEO" | "FRAME" | "TEXT";
        if (el.tagName === "IMG") {
          nodeType = "IMAGE";
          ...
          typography: hasTypography
            ? {
                ...
                font: {
                  ...
                  smoothing: (styles as any).webkitFontSmoothing || "auto",
                ...
                decoration: {
                  ...
                  thickness: (styles as any).textDecorationThickness || "auto",
                  underlineOffset:
                    (styles as any).textUnderlineOffset || "auto",
                  underlinePosition:
                    (styles as any).textUnderlinePosition || "auto",
                },
                effects: {
                  color: styles.color,
                  shadow: styles.textShadow,
                  transform: styles.textTransform,
                  stroke: (styles as any).webkitTextStroke || null,
                  strokeWidth: (styles as any).webkitTextStrokeWidth || null,
                  strokeColor: (styles as any).webkitTextStrokeColor || null,
                  fillColor: (styles as any).webkitTextFillColor || null,
                  gradientText: detectGradientText(styles).gradientData,
                },
                wrapping: {
                  textWrap: (styles as any).textWrap || null,
                  textWrapMode: (styles as any).textWrapMode || null,
                  textWrapStyle: (styles as any).textWrapStyle || null,
                },
    ...
    { capturePseudoElements, screenshotComplexOnly, renderEnv }
      ...
      // Timeout promise
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`DOM extraction timeout after ${DOM_EXTRACTION_TIMEOUT}ms`)), DOM_EXTRACTION_TIMEOUT)
      )
    ]);
  } catch (error) {
  ...
  for (const node of data.nodes) {
    const screenshot = await page.evaluate((id) => {
      return (window as any).__primaryScreenshots?.[id];
    }, node.domId || node.id);
    if (screenshot) {
  ...
  const { nodes: nodesWithRaster, stats: rasterStats } =
    await processTextRasterization(page, data.nodes, renderEnv);
  ...
  const lowConfidence = nodesWithRaster.filter(
    (n) => n.validation?.useFallback
  ).length;
  ...
  const flattenable = nodesWithRaster.filter(
    (n) => n.optimization?.canFlatten
  ).length;
  ...
  for (let i = 0; i < nodesWithRaster.length; i++) {
    ...
    const context = stackingResult.stackingContexts.find(ctx => 
      ctx.nodeIds.includes(node.id)
    );
    ...
    if (paintIndex !== -1) {
      ...
      if (!node.layout.stacking) node.layout.stacking = {};
  ...
  let screenshots: Record<string, EnhancedScreenshot> = {};
  let screenshotCandidates: any[] = [];
  if (captureScreenshots && !isAborted) {
    const candidatePool = (screenshotComplexOnly
      ? nodesWithNames.filter((n: any) => n.needsScreenshot)
      : nodesWithNames
    ).filter(shouldScreenshotNodeCandidate);

    const { nodes: nodesToScreenshot, stats: screenshotPrefilterStats } =
      prepareScreenshotCandidates(candidatePool);
    ...
    if (candidatePool.length !== nodesToScreenshot.length) {
      ...
      const skippedTotal =
        screenshotPrefilterStats.total - screenshotPrefilterStats.eligible;
      console.log(
        `[Phase 3] ⏭️  Prefiltered ${skippedTotal} screenshot candidates (hidden: ${skippedHidden}, zero-size: ${skippedZeroSize}, detached: ${skippedDetached})`
      );
    }

    screenshots = await captureElementScreenshots(
      page,
      nodesToScreenshot,
      renderEnv
    );

    const missingScreenshotNodes = nodesToScreenshot.filter(
      (node) => !screenshots[node.id]
    );

    if (
      missingScreenshotNodes.length > 0 &&
      primaryScreenshots?.page?.src
    ) {
      await applyPageScreenshotFallback(
        missingScreenshotNodes,
        primaryScreenshots.page,
        screenshots
      );
    }
  } else if (isAborted) {
  ...
  let states: Record<string, Record<string, EnhancedScreenshot>> = {};
  if (captureStates && !isAborted) {
    states = await captureInteractionStates(
      page,
      nodesWithNames.filter(
  ...
  const result: IRDocument = legacyMigrator.migrateFromExtractedData(legacyResult);
  
  // Compile and normalize IR for Figma-ready output
  console.log("[Phase 10] Starting IR compilation and normalization...");
  const compiledResult: IRDocument = compileIR(result);
