 ...
 */
export function parseEffectsToIR(styles) {
    const effects = {
 ...
 */
function parseBoxShadowToIR(boxShadow) {
    const shadows = [];
 ...
 */
function parseTextShadowToIR(textShadow) {
    const shadows = [];
 ...
 */
function parseSingleShadow(shadowStr, type) {
    try {
        ...
        const match = cleanShadow.match(/(-?[\d.]+(?:px|em|rem|%)?)\s+(-?[\d.]+(?:px|em|rem|%)?)\s+([\d.]+(?:px|em|rem|%)?)\s*(?:([\d.]+(?:px|em|rem|%)?)\s+)?(.+)/);
        if (!match)
            return null;
 ...
 */
function parseFiltersToIR(filter) {
    const filters = [];
 ...
 */
function parseBackdropFiltersToIR(backdropFilter) {
    const filters = [];
 ...
 */
export function analyzeTextShadowComplexity(textShadow) {
 ...
 */
export function analyzeFilterComplexity(filter) {
...
//# sourceMappingURL=effects-parser.js.map